...
#                              ~/.claude/my_vault/sparkle_ed25519_private_key.txt
...
if [[ -n "${SPARKLE_PRIVATE_KEY:-}" ]]; then
  KEY_SOURCE="env"
elif [[ -f "$HOME/.claude/my_vault/sparkle_ed25519_private_key.txt" ]]; then
  SPARKLE_PRIVATE_KEY="$(cat "$HOME/.claude/my_vault/sparkle_ed25519_private_key.txt")"
  KEY_SOURCE="vault"
else
