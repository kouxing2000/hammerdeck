...
Or build it from source (Xcode 16 or later; on macOS 14 also `brew install jq`):
...
Build through the script rather than a bare `swift build`: it links with the
linker flags that record the real macOS SDK, and a bare build on Xcode 27 does
not, which changes how AppKit lays some windows out.
...
- **Full Disk Access** -- emptying the Trash reads a folder macOS protects, and macOS never prompts for this one, so switch Hammerdeck on under Privacy & Security yourself: the rules engine (Empty the Trash).
...
Nothing is requested at install. First launch explains why the window features
need Accessibility and offers a button to grant it; most others ask when you
switch them on or first use them, and a feature you never enable never asks.
Full Disk Access is the exception: macOS never asks for it, so you grant it
yourself if you want the Empty the Trash rule. If a
feature does nothing, the pinned
[permission gotchas](https://github.com/kouxing2000/hammerdeck/issues/6) issue
covers the known causes.

## Build & test

```bash
scripts/app.sh start     # build CLua (vendored Lua 5.4.7) + the host, then launch it
...
Boot without grabbing hotkeys: `HAMMERDECK_NO_FIRSTRUN=1 scripts/app.sh start`.
