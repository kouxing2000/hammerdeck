...
- **Quick Sites** -- Jump to a favorite site -- focuses its tab if it's already open, opens it if not. Per site, pick the browser, a Chrome profile, and whether to open it as a standalone app window. cmd+<number> jumps straight to a row in the picker, and every site is also a menubar row you can give its own shortcut. <sub>Reaches: reads browser tabs, network, reads/writes files.</sub>
